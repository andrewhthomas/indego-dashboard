      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-2">
            ...
            {station.addressState} {station.addressZipCode}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <Badge
              variant={
                station.kioskPublicStatus === "Active" ? "default" : "secondary"
              }
            >
                ...
                <div className="grid grid-cols-4 gap-2">
                  {availableBikes.map((bike) => {
                    ...
                    return (
                      ...
                      >
                        <BikeIcon
                          className={`h-5 w-5 mb-1 ${bike.isElectric ? "text-yellow-600" : ""}`}
                        />
                        <span className="text-xs font-medium">
                          Dock {bike.dockNumber}
                        </span>
                        <span className="text-xs text-muted-foreground">
                          {getBikeTypeLabel(bike)}
                        </span>
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-muted-foreground">
              Occupied Docks ({unavailableBikes.length})
            </h3>
            <div className="text-sm text-muted-foreground">
