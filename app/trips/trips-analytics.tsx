...
export function TripsAnalytics() {
          ...
          <CardContent className="p-6">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                Please ensure the CSV file is properly loaded and the API is accessible.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && tripData?.stats && tripData.stats.totalTrips === 0 && selectedMonth !== 'all' && (
        <Card>
          <CardContent className="p-6">
            <div className="text-center text-muted-foreground">
              <div className="mb-2">
              ...
              <p className="text-sm">
                There are no bike trips recorded for the selected month. 
                Try selecting a different month or view all data.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {!loading && tripData?.stats && tripData.stats.totalTrips > 0 && (
        ...
        <TabsList>
          ...
          <TabsTrigger value="bikes">Bike Types</TabsTrigger>
          ...
          <Card>
            ...
            <CardContent className="h-[400px]">
              {loading ? (
                  ...
                  <div className="flex items-end justify-center space-x-1 h-48">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <Skeleton
                        key={i}
                        className={`w-3 ${
                          i >= 6 && i <= 9 ? 'h-32' : // Morning peak
                          i >= 17 && i <= 19 ? 'h-40' : // Evening peak
                          i >= 10 && i <= 16 ? 'h-16' : // Daytime
                          'h-8' // Night
                        }`}
                      />
                    ))}
                ...
                <HourlyDistributionChart data={tripData.stats.hourlyDistribution} />
              ) : (
                <div className="h-full flex items-center justify-center text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        ...
                        <span className="text-muted-foreground ml-1">
                          ({Math.round((tripData.stats.bikeTypeBreakdown.electric / tripData.stats.totalTrips) * 100)}%)
                        </span>
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        ...
                        <span className="text-muted-foreground ml-1">
                          ({Math.round((tripData.stats.bikeTypeBreakdown.standard / tripData.stats.totalTrips) * 100)}%)
                        </span>
                      </span>
                    </div>
                    <div className="pt-2 border-t">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {(tripData.stats.totalDistance / tripData.stats.totalTrips).toFixed(2)} mi
                        </span>
                      </div>
                    </div>
                  </div>
